 ...
 * [计算属性：表格数据]
 */
Tool.tableList = function (state, name) {
  const tableData = state[name]
  let arr = []
  const arrObj = {}
  /* 按 1,2,3,4 添加到对象 */
  for (const x in tableData) {
    let examine_result_0 = '1'
    for (let i = 0; i < tableData[x].length; i++) {
      ...
      if (val.is_delete !== '0') {
        ...
        // if (i === 0 && String(val.examine_result) === '1') {
        //   // 此仓库验货结果：合格
        //   break
        // }
        if (i === 0) {
          examine_result_0 = String(val.examine_result)
        } else {
          val.examine_result = examine_result_0
        }
        //
        val.key = i
 ...
 * [计算属性：不合格原因汇总]
 */
Tool.summary = function (state, name) {
  const { arrOut, arrIn, arrResult, arrResultType } = state
  const tableData = state[name]
  const obj = {}
  for (const x in tableData) {
    const arr = []
 ...
 * [计算属性：责任划分（计算文本框外的数字）]
 */
Tool.accountability = function (state, name) {
...
Tool.changeTime = function (state) {
  const { tableData_new, tableData_old } = state
  const arr_new = []
  const arr_old = []
  /* 提取：已选时间的毫秒数 */
  for (const x in tableData_new) {
    if (tableData_new[x][0] && tableData_new[x][0].examine_time) {
      const num = new Date(tableData_new[x][0].examine_time).getTime()
      if (num) {
        arr_new.push(num)
      }
    }
  ...
  /* 提取：已选时间的毫秒数 */
  for (const x in tableData_old) {
    if (tableData_old[x][0] && tableData_old[x][0].examine_time) {
      const num = new Date(tableData_old[x][0].examine_time).getTime()
      if (num) {
        arr_old.push(num)
      }
    }
  ...
  /* 取最近日期 */
  if (arr_new.length) {
    const minDay = new Date(arr_new[0])
    ...
    state.time_new = `${year}-${month}-${day}`
  } else {
    state.time_new = ''
  }
  if (arr_old.length) {
    const minDay = new Date(arr_old[0])
    const year = minDay.getFullYear()
    const month = minDay.getMonth() + 1 < 10 ? '0' + (minDay.getMonth() + 1) : minDay.getMonth() + 1
    const day = minDay.getDate() < 10 ? '0' + minDay.getDate() : minDay.getDate()
    state.time_old = `${year}-${month}-${day}`
  } else {
    state.time_old = ''
  }
}
...
Tool.changeCheckbox = function (state) {
  const resultArr_new = [] // 最终数组_新
  const resultArr_old = [] // 最终数组_旧
  /* 提取：现有数据 */
  const { arrResult, formData_new, formData_old, peopleList } = state
  const objFormData = { '内部': {}, '外部': {} } // objFormData = { '内部': { '业务岗': {单条数据} }, '外部': {} }
  /* 提取：类型占比_new */
  let inNum_new = 0
  let outNum_new = 0
  const formDataObj_new = {}
  formData_new.forEach(function (item) {
    if (item.typeNum && item.type === '内部') {
      inNum_new = item.typeNum
    }
    if (item.typeNum && item.type === '外部') {
      outNum_new = item.typeNum
    }
    formDataObj_new[item.job] = item
  })
  /* 提取：类型占比_old */
  let inNum_old = 0
  let outNum_old = 0
  const formDataObj_old = {}
  formData_old.forEach(function (item) {
    if (item.typeNum && item.type === '内部') {
      inNum_old = item.typeNum
    }
    if (item.typeNum && item.type === '外部') {
      outNum_old = item.typeNum
    }
    formDataObj_old[item.job] = item
  })
  /* 提取：勾选，所有涉及到的岗位（去重） */
  const { tableData_new, tableData_old, arrOut, arrIn } = state
  const textObj = { '内部': {}, '外部': {} } // textObj = { '内部': { '业务岗': '' }, '外部': {} }
  for (const x in tableData_new) {
    tableData_new[x].forEach(function (item) {
      if (arrResult[item.examine_result] === '不合格' && arrResult[tableData_new[x][0].examine_result] === '不合格') {
        //
      }
      //
  ...
  for (const x in tableData_old) {
    tableData_old[x].forEach(function (item) {
      if (arrResult[item.examine_result] === '不合格' && arrResult[tableData_old[x][0].examine_result] === '不合格') {
        //
      }
      //
      item.accountability.in.forEach(function (item) {
  ...
  inOutArr.forEach(function (itemItem, itemIndex) {
    itemItem.forEach(function (job, index, arr) {
      ...
      if (itemIndex === 0) {
        /* 内部 */
        const typeNum = index === 0 ? inNum_new : 0
        if (objFormData['内部'][job]) {
          // { index: 0, count: 3, type: '内部', typeNum: 0, job: '业务岗', jobNum: 0, list: [{ name: '业务1', num: 0 }, { name: '业务2', num: 0 }] }
          const obj = Object.assign({}, objFormData['内部'][job], { index: listIndex_new, count, typeNum })
          resultArr_new.push(obj)
        } else {
          if (formDataObj_new[job]) {
            const obj = Object.assign({}, formDataObj_new[job], { index: listIndex_new, count, typeNum })
            resultArr_new.push(obj)
          } else {
            const obj = { index: listIndex_new, count, type: '内部', typeNum, job: job, jobNum: 0, list: [] }
            if (peopleList[job]) {
        ...
        /* 外部 */
        const typeNum = index === 0 ? outNum_new : 0
        if (objFormData['外部'][job]) {
          const obj = Object.assign({}, objFormData['外部'][job], { index: listIndex_new, count, typeNum })
          resultArr_new.push(obj)
        } else {
          if (formDataObj_new[job]) {
            const obj = Object.assign({}, formDataObj_new[job], { index: listIndex_new, count, typeNum })
            ...
            const obj = { index: listIndex_new, count, type: '外部', typeNum, job: job, jobNum: 0, list: [] }
            resultArr_new.push(obj)
          }
        }
      }
  ...
  inOutArr.forEach(function (itemItem, itemIndex) {
    itemItem.forEach(function (job, index, arr) {
      ...
      if (itemIndex === 0) {
        /* 内部 */
        const typeNum = index === 0 ? inNum_old : 0
        if (objFormData['内部'][job]) {
          // { index: 0, count: 3, type: '内部', typeNum: 0, job: '业务岗', jobNum: 0, list: [{ name: '业务1', num: 0 }, { name: '业务2', num: 0 }] }
          const obj = Object.assign({}, objFormData['内部'][job], { index: listIndex, count, typeNum })
          resultArr_old.push(obj)
        } else {
          if (formDataObj_old[job]) {
            const obj = Object.assign({}, formDataObj_old[job], { index: listIndex, count, typeNum })
            resultArr_old.push(obj)
          } else {
        ...
        /* 外部 */
        const typeNum = index === 0 ? outNum_old : 0
        if (objFormData['外部'][job]) {
  ...
  /* 赋值 */
  state.formData_new = resultArr_new
  state.formData_old = resultArr_old
}
...
Tool.submitExamine_goods_detail = function (obj = {}, file, del_files = []) {
  ...
  for (const x in obj) {
      ...
      if (item.isLocal && item.is_delete === '0') {
        ...
        if (is_delete === '1') {
          ...
          if (examine_result !== null && !examine_result.length) {
            ...
            status = false
            textArr.push('验货结果')
          } else if (String(examine_result) === '1') {
            // 合格
            const { reason_type, reason_desc, ratio = 0, detail_explain, accountability: { in: inArr, out: outArr }, handle_mode } = item
        ...
        /* 报错 */
        if (!status) {
          error_detail.push(`第${i}条，${textArr.join('、')}`)
        }
        i++
      }
    })
    ...
    //   if (String(examine_result) === '0' && is_delete === '1') {
